...
public class CNLLTMODpairs extends AnalyticalModelODpairs{
	
	private final Scenario scenario;
	private final TransitSchedule ts;
	public CNLLTMODpairs(Network network, Population population,TransitSchedule ts, Scenario scenario,Map<String, Tuple<Double, Double>> timeBeans) {
		super(network, population,timeBeans,scenario);
		this.scenario=scenario;
		this.ts=ts;
	}
	
	public CNLLTMODpairs(String networkFileLoc,String populationFileLoc,TransitSchedule ts, Scenario scenario,HashMap<String,Tuple<Double,Double>> timeBean) {
	...
	protected TripChain getNewTripChain(Plan plan) {
		return new CNLLTMTripChain(plan,this.ts,this.scenario,network);
